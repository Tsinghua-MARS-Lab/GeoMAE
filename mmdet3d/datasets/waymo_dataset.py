...
class WaymoDataset(KittiDataset):
    ...
    def get_data_info(self, index):
        ...
        if not self.test_mode or test_save_gt:
...
@DATASETS.register_module()
class WaymoDatasetComplement(KittiDataset):
    """Waymo Dataset.
    ...
    def __init__(self,
        ...
        # to load a subset, just set the load_interval in the dataset config
        index_flag=torch.ones(len(self.data_infos),dtype=torch.bool)
        index_flag[::load_interval]=False
        index=torch.arange(len(self.data_infos),dtype=torch.int32)
        index=index[index_flag]
        data_infos = np.array(self.data_infos)
        self.data_infos = data_infos[index]
        if hasattr(self, 'flag'):
