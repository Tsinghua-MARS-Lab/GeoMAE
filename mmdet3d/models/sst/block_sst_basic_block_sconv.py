...
class WindowAttention(nn.Module):
    ...
    def forward(self, feat_2d, pos_dict, ind_dict, voxel_drop_level, key_padding_dict, drop_info):
        ...
        for name in feat_3d_dict:
            ...
            #print(out_feat_3d.shape,attn_map.shape)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1,
        ...
        self.block_id=layer_id//2
        self.layer_id=layer_id%2
        if self.block_id % 5 == 0 and self.layer_id == 1:
            self.sconv=spconv.SparseSequential(spconv.SubMConv2d(
            d_model, d_model, kernel_size=3, stride=1, padding=1,  bias=False, indice_key='sconv',
        ))
    ...
    def forward(
        ...
        src = sp_tensor.features
        src2 = self.win_attn(src, pos_dict, ind_dict, voxel_drop_level, key_padding_mask_dict, drop_info) #[N, d_model]
        src = src + self.dropout1(src2)
        src = self.norm1(src)
        # print('after norm1',src.dtype)


        src2 = self.linear2(self.dropout(self.activation(self.linear1(src))))
        src = src + self.dropout2(src2)
        sp_tensor.features = self.norm2(src)
        if self.block_id % 5 == 0 and self.layer_id == 1:
            sp_tensor=self.sconv(sp_tensor)
        # print('after norm2', src.dtype)
        return sp_tensor

class BlockShiftBlockSconv(nn.Module):
    ...
    def forward(
        ...
        for i in range(2):
            ...
            pos_dict = pos_dict_list[block_id]
            ind_dict = ind_dict_list[block_id]
            voxel_drop = voxel_drop_level_list[block_id]
            key_mask_dict = key_mask_dict_list[block_id]
