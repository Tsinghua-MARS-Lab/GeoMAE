...
class CenterHeadIoU(BaseModule):
    ...
    def __init__(self,
                 ...
                 num_heatmap_convs=2,
                 loss_iou=dict(type='GaussianFocalLoss', reduction='mean'),
                 loss_iou_reg=dict(type='GaussianFocalLoss', reduction='mean'),
        ...
        super(CenterHeadIoU, self).__init__(init_cfg=init_cfg)
        ...
        self.with_iou_reg = loss_iou_reg is not None
        if self.with_iou:
            self.crit_iou = build_loss(loss_iou)
        self.crit_iou_reg = None
        if self.with_iou_reg:
            self.crit_iou_reg = build_loss(loss_iou_reg)
    ...
    def get_targets_single(self, gt_bboxes_3d, gt_labels_3d):
        ...
        gt_bboxes_3d = torch.cat(
            (gt_bboxes_3d.gravity_center, gt_bboxes_3d.tensor[:, 3:self.bbox_coder.code_size]),
            dim=1).to(device)
    ...
    def loss(self, gt_bboxes_3d, gt_labels_3d, preds_dicts, **kwargs):
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            # print('task_id',preds_dict[0]['heatmap'].shape,
            #     heatmaps[task_id].shape,)
            ...
            if self.with_iou or self.with_iou_reg:
                batch_dim = torch.exp(torch.clamp(preds_dict[0]['dim'], min=-5, max=5))
                batch_dim = batch_dim.permute(0, 2, 3, 1).contiguous()
                batch_rot = preds_dict[0]['rot'].clone(); batch_rot = batch_rot.permute(0, 2, 3, 1).contiguous()
                # print(batch_rot.shape)
                batch_rots = batch_rot[..., 0:1]
                batch_rotc = batch_rot[..., 1:2]
                batch_reg = preds_dict[0]['reg'].clone().permute(0, 2, 3, 1).contiguous()
                batch_hei = preds_dict[0]['height'].clone().permute(0, 2, 3, 1).contiguous()
                batch_rot = torch.atan2(batch_rots, batch_rotc)

                batch, H, W, _ = batch_dim.size()
                print('batch_hei',batch_hei.shape)
                ...
                xs = xs * self.train_cfg['out_size_factor'] * voxel_size[0] + pc_range[0]
                ys = ys * self.train_cfg['out_size_factor'] * voxel_size[1] + pc_range[1]
...
o
        ...
        for i, (box_preds, cls_preds, cls_labels) in enumerate(
            ...
            print(self.num_classes,cls_labels.min(),cls_labels.max())
