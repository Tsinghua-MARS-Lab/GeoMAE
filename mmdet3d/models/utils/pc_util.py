...
def write_bbox(scene_bbox, out_filename):
    ...
    for i in range(len(scene_bbox)):
        #print(scene_bbox[i].shape)
        bbox=scene_bbox[i].cpu().numpy()
        scene.add_geometry(convert_box_to_trimesh_fmt(bbox))
...
def write_oriented_bbox(scene_bbox, out_filename):
    ...
    scene = trimesh.scene.Scene()
    for i in range(len(scene_bbox)):
        bbox=scene_bbox[i].pred_boxes._tensor.squeeze(0).cpu().numpy()
        scene.add_geometry(convert_oriented_box_to_trimesh_fmt(bbox))
