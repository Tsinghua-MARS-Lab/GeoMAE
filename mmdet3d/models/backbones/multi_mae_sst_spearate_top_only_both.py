...
class MultiMAESSTSPBoth(nn.Module):
    '''
    ...
    def __init__(
        ...
        self.decoder_pred_curv_low = nn.Linear(d_model[-1], 3, bias=True)
        self.decoder_pred_nor_low = nn.Linear(d_model[-1], 3, bias=True)
    ...
    def forward_decoder(self, visible_voxel_feat,coors,coors_mask,batch_size):
        ...
        reg_pred_nor_low = self.decoder_pred_nor_low(density_masked_output)
        reg_pred_curv_low = self.decoder_pred_curv_low(density_masked_output)
        ...
        if self.cls_sub_voxel:
            ...
            return reg_pred_low,reg_pred_med,reg_pred_top,reg_pred_nor_low,reg_pred_curv_low,cls_pred_low,cls_pred_med,
        else:
            return reg_pred_low,reg_pred_med,reg_pred_top,reg_pred_nor_low,reg_pred_curv_low
