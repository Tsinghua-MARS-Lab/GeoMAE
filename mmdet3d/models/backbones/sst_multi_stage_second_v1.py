...
class SSTMultiStageSecondv1(nn.Module):
    '''
    ...
    def __init__(
            ...
            for j in range(num_blocks[i]):
                blocks.append(
                    BasicShiftBlock(d_model[i], nhead[i], dim_feedforward[i],
                                dropout, activation, batch_first=False, block_id=j)
                )
            blocks_list.append(nn.Sequential(*blocks))
        ...
        self.sconv = spconv.SparseSequential(*sparse_blocks)
        self.blocks_list = nn.ModuleList(blocks_list)
    ...
    def forward(self, input_tuple):
        ...
        for i, blocks in enumerate(self.blocks_list):
            coors = voxel_info['coors'].int()
            for j,block in enumerate(blocks):
                output = block(output, pos_embed_list, ind_dict_list, voxel_drop_level_list,
                            padding_mask_list, self.drop_info, using_checkpoint=j in self.checkpoint_blocks)
            if i != self.num_stage-1:
                sp_tensor = spconv.SparseConvTensor(output, coors,
                                                    self.spatial_shape[i],
                                                    batch_size)
                sp_tensor=self.sconv[i](sp_tensor)
                output=sp_tensor.features
                coors=sp_tensor.indices.long()
                voxel_info = self.window_partition(coors, voxel_info,bev_shape=self.bev_shapes[i+1])
                voxel_info = self.get_voxel_keep_inds(voxel_info,len(self.shifts_list))
                voxel_keep_inds = voxel_info['voxel_keep_inds']


                voxel_num_before_drop = len(output)
                output = output[voxel_keep_inds]
                ...
                if self.debug:
                    coors_3d_dict_shift0 = flat2window(coors, voxel_info['voxel_drop_level_shift0'],
                                                       ind_dict_list[0], self.drop_info)
                    coors_2d = window2flat(coors_3d_dict_shift0, ind_dict_list[0])
                ...
                pos_embed_list = [
                    self.get_pos_embed(
                        _t,
                        voxel_info[f'coors_in_win_shift{k}'],
                        voxel_info[f'voxel_drop_level_shift{k}'],
                        voxel_feat.dtype,
                        voxel_info.get(f'voxel_win_level_shift{k}', None),
                        stage=i+1,
                    )
                    for k, _t in enumerate(ind_dict_list)
                    # 2-times for-loop, one for non-shifted layout, one for shifted layout
                ]
                voxel_drop_level_list = [voxel_info[f'voxel_drop_level_shift{k}'] for k in range(num_shifts)]
    ...
    def get_pos_embed(self, ind_dict, coors_in_win, voxel_drop_level, dtype, voxel_window_level,stage=0):
        '''
        Args:
        '''
        ...
        pos_length = self.d_model[stage] // 2
    ...
    @torch.no_grad()
    def window_partition(self, coors, voxel_info,bev_shape):

        shifts_list = self.shifts_list
        win_shape_x, win_shape_y = self.window_shape

        bev_shape_x ,bev_shape_y = bev_shape
