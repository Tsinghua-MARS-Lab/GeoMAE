...
class MultiMAESSTDensityTopOnly(nn.Module):
    '''
    ...
    def __init__(
        ...
        # self.decoder_pred_density_low = nn.Linear(d_model[-1], self.per_sub_voxel_num_low , bias=True)
        # self.decoder_pred_density_med = nn.Linear(d_model[-1], self.per_sub_voxel_num_med , bias=True)
        self.decoder_pred_density_top = nn.Linear(d_model[-1], 1, bias=True)
    ...
    def forward_decoder(self, visible_voxel_feat,coors,coors_mask,batch_size):
        ...
        # reg_pred_nor_low = self.decoder_pred_density_low(masked_output)
        # reg_pred_nor_med = self.decoder_pred_density_med(masked_output)
