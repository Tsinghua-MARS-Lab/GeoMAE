...
class MultiMAESSTSurf(nn.Module):
    '''
    ...
    def forward_decoder(self, visible_voxel_feat,coors,coors_mask,batch_size):
        ...
        reg_pred_nor_low = self.decoder_pred_nor_low(masked_output).view(-1, self.per_sub_voxel_num_low, 3)
        reg_pred_nor_med = self.decoder_pred_nor_med(masked_output).view(-1, self.per_sub_voxel_num_med, 3)
        reg_pred_nor_top = self.decoder_pred_nor_top(masked_output)
