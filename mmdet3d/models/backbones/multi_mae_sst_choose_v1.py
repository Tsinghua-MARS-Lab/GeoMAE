...
class MultiMAESSTCurvChoosev1(nn.Module):
    '''
    ...
    def __init__(
        ...
        if self.low:
            self.decoder_pred_nor_low = nn.Linear(d_model[-1], self.per_sub_voxel_num_low * 3, bias=True)
        if self.med:
            self.decoder_pred_nor_med = nn.Linear(d_model[-1], self.per_sub_voxel_num_med * 3, bias=True)
        if self.top:
            self.decoder_pred_nor_top = nn.Linear(d_model[-1], 3, bias=True)
    ...
    def forward_decoder(self, visible_voxel_feat, coors, coors_mask, batch_size):
        ...
        if self.low:
            reg_pred_nor_low = self.decoder_pred_density_low(masked_output).view(-1, self.per_sub_voxel_num_low, 3)
        else:
            reg_pred_nor_low = None
        if self.med:
            reg_pred_nor_med = self.decoder_pred_density_med(masked_output).view(-1, self.per_sub_voxel_num_med, 3)
        else:
            reg_pred_nor_med = None
        if self.top:
            reg_pred_nor_top = self.decoder_pred_density_top(masked_output)
        ...
        if self.cls_sub_voxel:
            ...
            return reg_pred_low, reg_pred_med, reg_pred_top, reg_pred_nor_low, reg_pred_nor_med, reg_pred_nor_top, cls_pred_low, cls_pred_med,
        else:
            return reg_pred_low, reg_pred_med, reg_pred_top, reg_pred_nor_low, reg_pred_nor_med, reg_pred_nor_top
