...
class MAESST(nn.Module):
    ...
    def __init__(
        ...
        self.per_sub_voxel_num = sub_voxel_ratio[0] * sub_voxel_ratio[1] * sub_voxel_ratio[2]
        self.decoder_pred = nn.Linear(d_model[-1],self.per_sub_voxel_num*3 , bias=True)
        if cls_sub_voxel:
            self.cls_pred = nn.Linear(d_model[-1],self.per_sub_voxel_num*2 , bias=True)
    ...
    def forward(self, voxel_feat, coors,coors_mask,batch_size):
        ...
        if self.cls_sub_voxel:
            reg_pred,cls_pred = self.forward_decoder(visible_output,coors,coors_mask,batch_size)
            return reg_pred,cls_pred
        else:
            reg_pred = self.forward_decoder(visible_output, coors, coors_mask, batch_size)
            return reg_pred
    ...
    def forward_decoder(self, visible_voxel_feat,coors,coors_mask,batch_size):
        ...
        reg_pred = self.decoder_pred(masked_output).view(-1,self.per_sub_voxel_num,3)
        if self.cls_sub_voxel:
            cls_pred = self.cls_pred(masked_output).view(-1,self.per_sub_voxel_num,2)
            return reg_pred,cls_pred
        else:
            return reg_pred
