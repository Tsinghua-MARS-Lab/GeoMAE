...
class MultiSubVoxelDynamicVoxelNetSpconvFinal(SingleStage3DDetector):
    ...
    def __init__(self,
                 ...
                 pretrained=None,
                 init_cfg=None):
        super(MultiSubVoxelDynamicVoxelNetSpconvFinal, self).__init__(
            backbone=backbone,
            neck=neck,
            bbox_head=bbox_head,
            train_cfg=train_cfg,
    ...
    def forward_train(self,
        ...
        if self.cls_sub_voxel:
            x,centroid_low,centroid_low_mask ,centroid_med,centroid_med_mask ,centroid_high,centroid_normal = \
                self.extract_feat(points,gt_bboxes_3d,gt_labels_3d, img_metas)
            reg_pred_low, reg_pred_med,reg_pred_high,cls_pred_low, cls_pred_med,nor_pred = x
            ...
            loss = self.forward_loss(centroid_low,centroid_low_mask ,centroid_med,centroid_med_mask ,centroid_high,
                                     reg_pred_low,reg_pred_med,reg_pred_high,cls_pred_low, cls_pred_med,nor_pred=nor_pred,centroid_normal=centroid_normal)
        else:
    ...
    def extract_feat(self, points: list,gt_bboxes_3d,gt_labels_3d, img_metas,vis=False):
        ...
        # points_coor=points[:3]
        batch_size=len(points)
        # for i in range(batch_size):
        #     points[i]=points[i][:,:3].contiguous()
        voxels,input_coors=self.input_voxelize(points)
        coors = self.final_voxelize(points)
        sub_coors_low = self.sub_voxelize_low(points)
        sub_coors_med = self.sub_voxelize_med(points)
        # print("！！！！！！！！！！！！"*50)
        # print()
        # print(voxels.shape)
        voxel_features, feature_coors = self.voxel_encoder(voxels, input_coors)
        ...
        if self.use_focal_mask is not None:
            ids_keep, ids_mask = self.get_focal_mask_index(centroid_voxel_coors_high, gt_bboxes_3d, gt_labels_3d)
        else:
            ids_keep,ids_mask=self.get_vanilla_mask_index(centroid_voxel_coors_high,batch_size)

        centroids_med_for_curv, centroid_mask_med_for_curv = self.get_multi_voxel_id_to_tensor_id_for_curv \
            (centroid_voxel_coors_high.long(), centroid_voxel_coors_med.long(), centroids_med, batch_size)
        ...
        out_inds, indice_num_per_loc, pair, pair_bwd,           \
            ...
            indices=centroid_voxel_coors_high,
        ...
        centroids_normal_low = self.cal_regular_voxel_curv_around(centroids_med_for_curv, centroid_mask_med_for_curv,
                                                                  centroids_high, pair.long())
        # if vis:
        ...
        #
        centroids_low,centroid_mask_low,centroids_med,centroid_mask_med,input_ids_keep \
            =self.get_multi_voxel_id_to_tensor_id(centroid_voxel_coors_high.long(),centroid_voxel_coors_low.long(),centroid_voxel_coors_med.long(),
                                                  centroids_low,centroids_med,ids_mask,ids_keep,feature_coors.long(),batch_size)
        with torch.no_grad():
            centroids_high=centroids_high[ids_mask]
            centroids_normal_low = centroids_normal_low[ids_mask]

        mask_coors=centroid_voxel_coors_high[ids_mask]
        ...
        x = self.backbone(voxel_features[input_ids_keep],feature_coors[input_ids_keep],mask_coors,batch_size)

        if self.vis:
            return x, centroids_low, centroid_mask_low,centroids_med,centroid_mask_med,centroids_high,mask_coors[:,1:]
        else:
            return x, centroids_low, centroid_mask_low,centroids_med,centroid_mask_med,centroids_high,centroids_normal_low

    @torch.no_grad()
    @force_fp32()
    def cal_regular_voxel_curv_around(self,centroid_low,centroid_low_mask,voxel_centroid,indice_pairs):
        ...
        # print()
        # print('voxel_centroid',voxel_centroid.shape,voxel_num,indice_pairs.shape)

        #voxel_centroid_around=voxel_centroid.view(voxel_num,1,3).repeat(1,sub_voxel_num*around_num,1)
    ...
    @torch.no_grad()
    @force_fp32()
    def final_voxelize(self, points):
        """Apply dynamic voxelization to points.
    ...
    @torch.no_grad()
    @force_fp32()
    def input_voxelize(self, points):
        """Apply dynamic voxelization to points.
        ...
        # dynamic voxelization only provide a coors mapping
        for res in points:
            res_coors = self.input_voxel_layer(res)
            coors.append(res_coors)
    ...
    @torch.no_grad()
    def get_multi_voxel_id_to_tensor_id(self, voxel_coors,voxel_coors_low,voxel_coors_med,voxel_centroids_low,voxel_centroids_med,ids_masked,ids_keep,input_coors, batch_size, ):
        # TODO: this version doesn't support the ori voxel's height isn't the whole pillar
        voxel_num = voxel_coors.shape[0]
        grid_shape = self.grid_size[0] * self.grid_size[1] * self.grid_size[2]
        ...
        hash_table = voxel_coors.new_zeros(batch_size * grid_shape, dtype=torch.int64)
        hash_table_flag = voxel_coors.new_zeros(batch_size * grid_shape, dtype=torch.bool)
        voxel_id = torch.arange(voxel_coors.shape[0], device=voxel_coors.device)
        tensor_id = voxel_coors[:, 0] * grid_shape + voxel_coors[:, 2] * self.grid_size[1] + voxel_coors[:, 3]
        hash_table[tensor_id] = voxel_id

        hash_table_flag[tensor_id[ids_keep]] = True
        ...
        tensor_id_input_keep = input_coors[:, 0] * grid_shape + input_coors[:, 2] // self.input_voxel_ratio[1] * \
                    self.grid_size[1] + input_coors[:, 3] // self.input_voxel_ratio[2]
        tensor_id_input_keep = hash_table_flag[tensor_id_input_keep]


        return centroid_target_low, centroid_target_mask_low,centroid_target_med,centroid_target_mask_med,tensor_id_input_keep
    ...
    def forward_loss(self,centroid_low,centroid_low_mask ,centroid_med,centroid_med_mask ,centroid_high,
                                     reg_pred_low,reg_pred_med,reg_pred_high,cls_pred_low=None, cls_pred_med=None,nor_pred=None,centroid_normal=None):
        ...
        loss_nor = (nor_pred - centroid_normal) ** 2
        loss_nor = loss_nor.mean(dim=-1)
        loss_nor = loss_nor.sum() / loss_nor.shape[0] * self.loss_ratio_nor
        ...
        if self.cls_sub_voxel:
            ...
            loss = dict(loss_nor=loss_nor,loss_centroid_low=loss_reg_low,loss_centroid_med=loss_reg_med,loss_centroid_top=loss_reg_top,
