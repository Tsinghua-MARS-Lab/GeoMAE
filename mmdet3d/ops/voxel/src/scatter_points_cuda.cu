...
std::vector<at::Tensor> dynamic_point_to_voxel_forward_gpu(
  ...
  // the first element of out_coors is always (-1,-1,-1) and should be removed TODO: judge the first element is equal to -1.
  // std::cout<<out_coors[0][0].item<int>()<<std::endl;
  if(out_coors[0][0].item<int>() == -1){
