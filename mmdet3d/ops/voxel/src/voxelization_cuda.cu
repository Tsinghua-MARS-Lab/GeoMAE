...
template <typename T, typename T_int>
__global__ void assign_flag_to_voxel(const int nthreads,
                                      T_int* point_to_voxelidx,
                                      T_int* coor_to_voxelidx, T* voxels,
                                      const int max_points
                                      ) {
  CUDA_1D_KERNEL_LOOP(thread_idx, nthreads) {
    ...
    int num = point_to_voxelidx[thread_idx];
    int voxelidx = coor_to_voxelidx[thread_idx];
    if (num > -1 && voxelidx > -1) {
      auto voxels_offset =
          voxels + voxelidx * max_points;

      voxels_offset[num] = true;
    }
  }
}
...
int hard_voxelize_with_flag_gpu(const at::Tensor& points, at::Tensor& voxels,at::Tensor& voxels_flag,
                      at::Tensor& coors, at::Tensor& num_points_per_voxel,
                      const std::vector<float> voxel_size,
                      const std::vector<float> coors_range,
                      const int max_points, const int max_voxels,
  ...
  auto flag_output_size = num_points;
  dim3 flag_cp_grid(std::min(at::cuda::ATenCeilDiv(flag_output_size, 512), 4096));
  dim3 flag_cp_block(512);
   AT_DISPATCH_ALL_TYPES(
      points.scalar_type(), "assign_flag_to_voxel", ([&] {
        assign_flag_to_voxel<bool, int>
            <<<flag_cp_grid, flag_cp_block, 0, at::cuda::getCurrentCUDAStream()>>>(
                flag_output_size,
                point_to_voxelidx.contiguous().data_ptr<int>(),
                coor_to_voxelidx.contiguous().data_ptr<int>(),
                voxels_flag.contiguous().data_ptr<bool>(), max_points);
      }));
  //   cudaDeviceSynchronize();
  //   AT_CUDA_CHECK(cudaGetLastError());
