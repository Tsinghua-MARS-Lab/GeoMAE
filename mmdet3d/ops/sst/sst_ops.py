...
@torch.no_grad()
def windowMax(feat, voxel_drop_lvl, flat2win_inds_dict,id_map_dict, drop_info,num_windows):
    '''
    Args:
        feat: shape=[N, C], N is the voxel num in the batch.
        voxel_drop_lvl: shape=[N, ]. Indicates drop_level of the window the voxel belongs to.
    ...
    device = feat.device
    feat_dim = feat.shape[-1]
    max_features=feat.new_zeros((num_windows,feat_dim))
    ...
    for dl in drop_info:
        ...
        if this_inds.max() >= num_windows * max_tokens:
            set_trace()
        feat_3d[this_inds] = feat_this_dl
        max_features[id_map_dict[dl]] = feat_3d.reshape((num_windows, max_tokens, feat_dim)).max(dim=1)[0]

    return max_features


def flat2window_aware(feat,feat_indice, voxel_drop_lvl, flat2win_inds_dict, drop_info):
    '''
    Args:
        feat: shape=[N, C], N is the voxel num in the batch.
        voxel_drop_lvl: shape=[N, ]. Indicates drop_level of the window the voxel belongs to.
    ...
    for dl in drop_info:
        ...
        feat_this_dl = feat[dl_mask]
        feat_indice_this_dl = feat_indice[dl_mask]
        ...
        feat_3d_dict[dl] = feat_3d
        with torch.no_grad():
            feat_3d_indice = torch.zeros((num_windows * max_tokens), dtype=feat_indice.dtype, device=device,
                                         requires_grad=False)
            feat_3d_indice[this_inds] = feat_indice_this_dl
            feat_3d_indice = feat_3d_indice.reshape((num_windows, max_tokens))
            feat_3d_indice_dict[dl] = feat_3d_indice

    return feat_3d_dict, feat_3d_indice_dict
