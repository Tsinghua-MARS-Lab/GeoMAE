...
int window_focal_attention_with_hash_wrapper(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int attend_range, int hash_size,
    ...
    CHECK_INPUT(v_indices_tensor);
    CHECK_INPUT(xyz_to_vidx_tensor);
    CHECK_INPUT(choose_indices_tensor);


    int *attend_indices = attend_indices_tensor.data<int>();
    const int *v_indices = v_indices_tensor.data<int>();
    const int *xyz_to_vidx = xyz_to_vidx_tensor.data<int>();
    const int *choose_indices =choose_indices_tensor.data<int>();

    window_focal_attention_with_hash_kernel_launcher(x_max, y_max, z_max, num_voxels, attend_size, attend_range, hash_size,
...
int window_focal_attention_with_hash_test_wrapper(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int attend_range, int hash_size,
    ...
    window_focal_attention_with_hash_kernel_test_launcher(x_max, y_max, z_max, num_voxels, attend_size, attend_range, hash_size,
                                                        attend_indices,choose_indices, v_indices, xyz_to_vidx);
    return 1;
}