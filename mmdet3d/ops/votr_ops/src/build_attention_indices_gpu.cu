...
__global__ void window_focal_attention_with_hash_kernel(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int attend_range, int hash_size,
                                                        int *attend_indices,const int*choose_indices, const int *v_indices, const int *xyz_to_vidx) {
    /*
        attend_indices: [num_voxels, attend_size] for gather attend indices
        v_indices: [num_voxels, 4] bs + zyx indices of voxels
        xyz_to_vidx: [bs, x_max, y_max, z_max] voxel coordinates to voxel indices
    */
    ...
    //    int num_samples = 0;
    for (int i=0;i<attend_size;i++){
        int sz_idx=choose_indices[i*3+0] + z_idx;
        int sy_idx=choose_indices[i*3+1] + y_idx;
        int sx_idx=choose_indices[i*3+2] + x_idx;
        if (sz_idx >= z_max || sz_idx < 0 || sy_idx >= y_max || sy_idx < 0 || sx_idx >= x_max || sx_idx < 0) continue;
        int skey = sx_idx * y_max * z_max + sy_idx * z_max + sz_idx;
        int sv_idx = hash_table_find(skey, hash_size, xyz_to_vidx);
        //        if (sv_idx != EMPTY_KEY) { // found non-empty index
        //            if (num_samples >= attend_size) return; // full and return
        attend_indices[th_idx * attend_size + i] = sv_idx;
...
void window_focal_attention_with_hash_kernel_launcher(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int attend_range, int hash_size,
                                                        int *attend_indices,const int*choose_indices, const int *v_indices, const int *xyz_to_vidx){
    ...
    window_focal_attention_with_hash_kernel<<<blocks, threads>>>(x_max, y_max, z_max, num_voxels, attend_size, attend_range, hash_size,
                                                                    attend_indices,choose_indices, v_indices, xyz_to_vidx);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
        fprintf(stderr, "CUDA kernel failed : %s\n", cudaGetErrorString(err));
...
__global__ void window_focal_attention_with_hash_test_kernel(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int attend_range, int hash_size,
                                                        int *attend_indices,const int*choose_indices, const int *v_indices, const int *xyz_to_vidx) {
    /*
        attend_indices: [num_voxels, attend_size] for gather attend indices
        v_indices: [num_voxels, 4] bs + zyx indices of voxels
...
void window_focal_attention_with_hash_kernel_test_launcher(int x_max, int y_max, int z_max, int num_voxels, int attend_size, int attend_range, int hash_size,
                                                        int *attend_indices,const int*choose_indices, const int *v_indices, const int *xyz_to_vidx){
    ...
    window_focal_attention_with_hash_test_kernel<<<blocks, threads>>>(x_max, y_max, z_max, num_voxels, attend_size, attend_range, hash_size,
                                                                    attend_indices,choose_indices, v_indices, xyz_to_vidx);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
