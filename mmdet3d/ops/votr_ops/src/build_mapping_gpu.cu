...
__global__ void build_mapping_with_hash_test_kernel(int x_max, int y_max, int z_max, int num_voxels, int hash_size,
                                                const int *v_indices, const int *v_bs_cnt, int *xyz_to_vidx) {
    /*
        v_indices: [N1+N2, 4] bs zyx indices of voxels
        v_bs_cnt: [bs] num_voxels in each sample
    ...
    */
    ...
    // key -> [x_max, y_max, z_max] value -> v_idx
    int key = x_idx * y_max * z_max + y_idx * z_max + z_idx;
    hash_table_insert(key, th_idx, hash_size, xyz_to_vidx);

    return;
}

void build_mapping_with_hash_kernel_test_launcher(int x_max, int y_max, int z_max, int num_voxels, int hash_size,
                                                const int *v_indices, const int *v_bs_cnt, int *xyz_to_vidx){
    ...
    build_mapping_with_hash_test_kernel<<<blocks, threads>>>(x_max, y_max, z_max, num_voxels, hash_size,
                                                            v_indices, v_bs_cnt, xyz_to_vidx);
    // cudaDeviceSynchronize();  // for using printf in kernel function
    err = cudaGetLastError();
    if (cudaSuccess != err) {
