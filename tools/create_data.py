...
def nuscenes_ssl_data_prep(root_path,
                       info_prefix,
                       version,
                       dataset_name,
                       out_dir,
    ...
    Args:
        ...
        max_sweeps (int, optional): Number of input consecutive frames.
            Default: 10
    """
    nuscenes_ssl_converter.create_nuscenes_infos(
        root_path, info_prefix, version=version, max_sweeps=max_sweeps)

    if version == 'v1.0-test':
        info_test_path = osp.join(root_path, f'{info_prefix}_infos_test.pkl')
        nuscenes_converter_ssl.export_2d_annotation(
            root_path, info_test_path, version=version)
        return
...
if __name__ == '__main__':
    ...
    elif args.dataset == 'nuscenes_ssl' and args.version != 'v1.0-mini':
        train_version = f'{args.version}-trainval'
        nuscenes_data_prep(
            root_path=args.root_path,
            info_prefix=args.extra_tag,
            version=train_version,
            dataset_name='NuScenesDatasetSSL',
            out_dir=args.out_dir,
            max_sweeps=args.max_sweeps)
